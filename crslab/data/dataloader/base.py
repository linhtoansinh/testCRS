...
class BaseDataLoader(ABC):
    ...
    def get_data(self, batch_fn, batch_size, shuffle=True, process_fn=None, flag = False):
        ...
        for start_idx in tqdm(range(batch_num)):
            ...
            if flag: print("batch index: ", batch_idx)
            batch = [dataset[idx] for idx in batch_idx]
            if flag: print(len(batch))
    ...
    def get_rec_data(self, batch_size, shuffle=True):
        ...
        """
        return self.get_data(self.rec_batchify, batch_size, shuffle, self.rec_process_fn, flag = True)
